...
export default function SerberAlatBerat() {
  ...
  const filteredData = tableData
    .filter((SerberAlatBerat) =>
      SerberAlatBerat.noRegistrasi.toLowerCase().includes(search.toLowerCase())
    )
    .slice(0, rows);
        ...
        <div className="min-w-[1102px]">
          <Table>
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {filteredData.map((SerberAlatBerat) => (
                <TableRow key={SerberAlatBerat.idSerberalatberat}>
                  <TableCell className="px-5 py-4 text-start text-theme-sm font-medium text-gray-800 dark:text-white/90">
                    {SerberAlatBerat.noRegistrasi}
                  </TableCell>
                  <TableCell className="px-4 py-3 text-start text-theme-sm text-gray-500 dark:text-gray-400">
                    {SerberAlatBerat.oliMesin}
                  </TableCell>
                  <TableCell className="px-4 py-3 text-start text-theme-sm text-gray-500 dark:text-gray-400">
                    {SerberAlatBerat.filterOliMesin}
                  </TableCell>
                  <TableCell className="px-4 py-3 text-start text-theme-sm text-gray-500 dark:text-gray-400">
                    <div className="flex items-center gap-2">
                      <EditButton
                        onClick={() =>
                          console.log("Edit", SerberAlatBerat.idSerberalatberat)
                        }
                      />
                    </div>
                  </TableCell>
