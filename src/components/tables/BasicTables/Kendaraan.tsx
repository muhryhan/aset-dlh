// pages/Kendaraan.tsx atau sesuai lokasi kamu
import {
  Table,
  TableBody,
  TableCell,
  TableHeader,
  TableRow,
} from "../../ui/table";
import ActionButtons from "../../actionbuttons"; // sesuaikan path-nya

interface Order {
  id: number;
  merek: string;
  nomorPolisi: string;
  nomorMesin: string;
  tahunPembuatan: string;
  kategori: string;
  kondisi: string;
}

const tableData: Order[] = [
  {
    id: 1,
    merek: "Honda MVP1",
    nomorPolisi: "DN 4442 DF",
    nomorMesin: "W04ADT-RR09037",
    tahunPembuatan: "2024",
    kategori: "R4",
    kondisi: "Baik",
  },
  {
    id: 2,
    merek: "Suzuki",
    nomorPolisi: "DN 2345 DF",
    nomorMesin : "W04ADT-RR09037",
    tahunPembuatan: "2021",
    kategori: "R2",
    kondisi: "Kurang Baik",
  },
];

export default function Kendaraan() {
  return (
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
      <div className="max-w-full overflow-x-auto">
        <div className="min-w-[1102px]">
          <Table>
            <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
              <TableRow>
                {["Merek", "No Polisi", "No Mesin", "Tahun Pembuatan", "Kategori", "Kondisi", "Aksi"].map((title) => (
                  <TableCell
                    key={title}
                    isHeader
                    className="px-5 py-3 font-medium text-start text-gray-500 text-theme-xs dark:text-gray-400"
                  >
                    {title}
                  </TableCell>
                   ))}
                   </TableRow>
              </TableHeader>

              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {tableData.map((order) => (
                  <TableRow key={order.id}>
                    <TableCell className="px-5 py-3 text-start text-theme-sm text-gray-800 dark:text-white/90">
                      {order.merek}
                    </TableCell>
                    <TableCell className="px-5 py-3 text-start text-theme-sm text-gray-500 dark:text-gray-400">
                      {order.nomorPolisi}
                    </TableCell>
                    <TableCell className="px-5 py-3 text-start text-theme-sm text-gray-500 dark:text-gray-400">
                      {order.nomorMesin}
                    </TableCell>
                    <TableCell className="px-5 py-3 text-start text-theme-sm text-gray-500 dark:text-gray-400">
                      {order.tahunPembuatan}
                    </TableCell>
                    <TableCell className="px-5 py-3 text-start text-theme-sm text-gray-500 dark:text-gray-400">
                      {order.kategori}
                    </TableCell>
                    <TableCell className="px-5 py-3 text-start text-theme-sm text-gray-500 dark:text-gray-400">
                      {order.kondisi}
                    </TableCell>
                    <TableCell className="px-5 py-3 text-start text-theme-sm text-gray-500 dark:text-gray-400">
                      <ActionButtons
                        onView={() => console.log("Lihat", order.id)}
                        onEdit={() => console.log("Edit", order.id)}
                        onDelete={() => console.log("Hapus", order.id)}
                      />
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
          </Table>
        </div>
      </div>
    </div>
  );
}
