...
import { SerberKendaraanData } from "../tables/PeriodicService/SerberKendaraanTable";
...
export default function SerberKendaraanForm({
  ...
  const isEdit = !!initialData?.id_serberkendaraan;
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        oli_mesin: initialData.oli_mesin
          ? localDate(initialData.oli_mesin)
          : "",
        filter_oli_mesin: initialData.filter_oli_mesin
          ? localDate(initialData.filter_oli_mesin)
          : "",
        oli_gardan: initialData.oli_gardan
          ? localDate(initialData.oli_gardan)
          : "",
        oli_transmisi: initialData.oli_transmisi
          ? localDate(initialData.oli_transmisi)
          : "",
        ban: initialData.ban ? localDate(initialData.ban) : "",
      });
    }
  }, [initialData]);

  const handleDateChange = (field: keyof typeof formData, date: Date[]) => {
    const selected = date[0] ? localDate(date[0]) : "";
